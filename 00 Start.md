---
typ: moc
aliases: [Home, MOC, Start]
tagi: [moc, ai-engineering]
---

# AI Engineering — baza wiedzy

Vault gromadzi trwałą wiedzę o projektowaniu systemów AI. Notatki powinny być niezależne od jednego kursu lub projektu i wskazywać źródła pierwotne.

## Mapa

### Modele i narzędzia

- [[Jev]]
- [[TypeSafe — przewodnik praktyczny]]

### Architektura i Kontekst

- [[Interpretable Context Methodology]]
- [[Context Compaction]]
- [[Bezpieczny punkt kompaktowania]]
- [[Harness]]
- [[Praca z harnessem]]
- [[Firstmate i Agenci Wykonawczy]]
- [[Prompt Architecture]]

### Metryki i Ewaluacja

- [[Rework Rate]]
- [[Eval Set z realnych sesji]]
- [[Selektywna weryfikacja kodu]]
- [[Stabilność modeli i przestrzeganie promptu]]
- [[Weryfikator]]

### Źródła

- [[TypeSafe i Jev — wywiad z Diogo Almeidą]]
- [[Kun Chen — Baza Wskazówek i Komentarzy]]
- [[HamelHusain — Baza Wskazówek i Komentarzy]]
- [[simonw — Baza Wskazówek i Komentarzy]]
- [[karminski3 — Baza Wskazówek i Komentarzy]]
- [[teortaxesTex — Baza Wskazówek i Komentarzy]]
- [[DrJimFan — Baza Wskazówek i Komentarzy]]
- [[JustinLin610 — Baza Wskazówek i Komentarzy]]
- [[Rekomendowani praktycy AI Engineering]]

## Reguła kwalifikacji

1. Wiedza potrzebna głównie do pracy w Bielik Bootcamp trafia do vaulta Bielika.
2. Wiedza przydatna niezależnie od bootcampu trafia tutaj.
3. Gdy temat dotyczy obu obszarów, pełna notatka żyje tutaj, a vault projektu może zawierać krótką notatkę zastosowania bez duplikowania treści.
